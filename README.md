...
## Architecture
AeroGlass runs a Baseline Dual-Layer BiLSTM alongside a custom Temporal Attention BiLSTM. While the baseline model operates as an uninterpretable "Black Box," the AeroGlass model extracts pre-computed SHAP matrices to isolate the mechanical stressors driving a failure (e.g., thermal stress vs. rotational mechanics). The front-end is engineered using Streamlit and Plotly, utilizing mmap_mode and @st.cache_resource to fluidly load and visualize heavy multi-dimensional tensor arrays without memory bloat.
